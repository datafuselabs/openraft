    ...
    Self: Wrapper<C, T>,
{
    ...
    async fn defensive_no_dirty_log(&mut self) -> Result<(), StorageError<C::NodeType>> {
        if !self.is_defensive() {
            return Ok(());
        }
    ...
    /// voted_for.
    async fn defensive_incremental_vote(
        &mut self,
    ...
    async fn defensive_consecutive_input(&self, entries: &[&Entry<C>]) -> Result<(), StorageError<C::NodeType>> {
        if !self.is_defensive() {
            return Ok(());
        }
    ...
    async fn defensive_nonempty_input(&self, entries: &[&Entry<C>]) -> Result<(), StorageError<C::NodeType>> {
        if !self.is_defensive() {
            return Ok(());
        }
    ...
    async fn defensive_append_log_id_gt_last(
        &mut self,
        entries: &[&Entry<C>],
    ) -> Result<(), StorageError<C::NodeType>> {
        if !self.is_defensive() {
    ...
    async fn defensive_purge_applied_le_last_applied(
        &mut self,
        upto: LogId<<C::NodeType as NodeType>::NodeId>,
    ) -> Result<(), StorageError<C::NodeType>> {
        let (last_applied, _) = self.inner().last_applied_state().await?;
    ...
    async fn defensive_delete_conflict_gt_last_applied(
        &mut self,
        since: LogId<<C::NodeType as NodeType>::NodeId>,
    ) -> Result<(), StorageError<C::NodeType>> {
        let (last_applied, _) = self.inner().last_applied_state().await?;
    ...
    async fn defensive_apply_log_id_gt_last(&mut self, entries: &[&Entry<C>]) -> Result<(), StorageError<C::NodeType>> {
        if !self.is_defensive() {
            return Ok(());
        }
