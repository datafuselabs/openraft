...
impl<C: RaftTypeConfig, N: RaftNetworkFactory<C>, S: RaftStorage<C>> RaftCore<C, N, S> {
    ...
    async fn do_main(&mut self) -> Result<(), Fatal<C::NodeId>> {
        ...
        let has_log = if self.engine.state.last_log_id.is_some() {
            "has_log"
        } else {
            "no_log"
        };

        let single = if self.engine.state.effective_membership.is_single() {
            "single"
        } else {
            "multi"
        };

        let is_voter = if self.engine.state.effective_membership.membership.is_member(&self.id) {
            "voter"
        } else {
            "learner"
        };
