...
where C: RaftTypeConfig
{
    ...
    async fn get_membership(&mut self) -> Result<Vec<EffectiveMembership<C::NodeId>>, StorageError<C::NodeId>> {
        let (_, sm_mem) = self.last_applied_state().await?;

        let sm_mem_next_index = sm_mem.log_id.next_index();

        let mut log_mem = self.last_membership_in_log(sm_mem_next_index).await?;
        ...
        let mut res = vec![sm_mem];
        res.append(&mut log_mem);
    ...
    async fn get_initial_state(&mut self) -> Result<RaftState<C::NodeId>, StorageError<C::NodeId>> {
        ...
        let memberships = self.get_membership().await?;
        ...
        Ok(RaftState {
            ...
            log_ids,
            effective_membership: Arc::new(memberships.last().unwrap().clone()),
            committed_membership: Arc::new(memberships.first().unwrap().clone()),
