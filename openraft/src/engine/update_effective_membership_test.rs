...
fn m23_45() -> Membership<TestNodeType> {
    Membership::<TestNodeType>::new(vec![btreeset! {2,3}], Some(btreeset! {4,5}))
}
...
fn m4_356() -> Membership<TestNodeType> {
    Membership::<TestNodeType>::new(vec![btreeset! {4}], Some(btreeset! {3,5,6}))
}
