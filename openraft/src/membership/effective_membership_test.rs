...
fn test_effective_membership_majority() -> anyhow::Result<()> {
    {
        let m12345 = Membership::<TestNodeType>::new(vec![btreeset! {1,2,3,4,5 }], None);
    ...
    {
        let m12345_678 = Membership::<TestNodeType>::new(vec![btreeset! {1,2,3,4,5 }, btreeset! {6,7,8}], None);
