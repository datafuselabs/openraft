...
impl NodeType for TestNodeType {
    ...
    type NodeData = BTreeMap<String, String>;
}
...
fn test_membership_summary() -> anyhow::Result<()> {
    ...
    let m = Membership::<TestNodeType>::new(vec![btreeset! {1,2}, btreeset! {3}], None);
    assert_eq!("members:[{1,2},{3}],learners:[]", m.summary());

    let m = Membership::<TestNodeType>::new(vec![btreeset! {1,2}, btreeset! {3}], Some(btreeset! {4}));
    assert_eq!("members:[{1,2},{3}],learners:[4]", m.summary());

    let m = Membership::<TestNodeType>::with_nodes(vec![btreeset! {1,2}, btreeset! {3}], btreemap! {
        1=>node("127.0.0.1", "k1"),
        2=>node("127.0.0.2", "k2"),
        3=>node("127.0.0.3", "k3"),
        4=>node("127.0.0.4", "k4"),
...
fn test_membership() -> anyhow::Result<()> {
    let m1 = Membership::<TestNodeType>::new(vec![btreeset! {1}], None);
    let m123 = Membership::<TestNodeType>::new(vec![btreeset! {1,2,3}], None);
    let m123_345 = Membership::<TestNodeType>::new(vec![btreeset! {1,2,3}, btreeset! {3,4,5}], None);
...
fn test_membership_with_learners() -> anyhow::Result<()> {
    ...
    {
        let m1_2 = Membership::<TestNodeType>::new(vec![btreeset! {1}], Some(btreeset! {2}));
    ...
    {
        let s1_2 = Membership::<TestNodeType>::new(vec![btreeset! {1,2,3}, btreeset! {5,6,7}], Some(btreeset! {3,4,5}));
...
fn test_membership_add_learner() -> anyhow::Result<()> {
    ...
    let m_1_2 = Membership::<TestNodeType>::with_nodes(
    ...
    assert_eq!(
        Membership::<TestNodeType>::with_nodes(
            vec![btreeset! {1}, btreeset! {2}],
    ...
    {
        let m_1_2 = Membership::<TestNodeType>::new(vec![btreeset! {1}, btreeset! {2}], None);
...
fn test_membership_extend_nodes() -> anyhow::Result<()> {
    ...
    let ext = |a, b| Membership::<TestNodeType>::extend_nodes(a, &b);
...
fn test_membership_with_nodes() -> anyhow::Result<()> {
    ...
    let m = |nodes| Membership::<TestNodeType>::with_nodes(vec![btreeset! {1}, btreeset! {2}], nodes);
...
fn test_membership_next_safe() -> anyhow::Result<()> {
    ...
    let m1 = Membership::<TestNodeType>::new(vec![c1()], None);
    let m2 = Membership::<TestNodeType>::new(vec![c2()], None);
    let m12 = Membership::<TestNodeType>::new(vec![c1(), c2()], None);
    let m23 = Membership::<TestNodeType>::new(vec![c2(), c3()], None);
    ...
    let m23_with_learners_old = Membership::<TestNodeType>::new(vec![c2(), c3()], Some(old_learners()));
    let m23_with_learners_new = Membership::<TestNodeType>::new(vec![c3()], Some(learners()));
...
fn test_membership_next_safe_with_nodes() -> anyhow::Result<()> {
    ...
    {
        let without_nodes = Membership::<TestNodeType>::new(vec![c1(), c2()], None);
    ...
    {
        let with_node_infos = Membership::<TestNodeType>::with_nodes(
            vec![c1(), c2()],
            btreemap! {1=>Some(node("1")), 2=>Some(node("2"))},
        )?;
