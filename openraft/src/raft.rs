...
struct RaftInner<C: RaftTypeConfig, N: RaftNetworkFactory<C>, S: RaftStorage<C>> {
    ...
    core_result: async_lock::Mutex<Result<(), Fatal<C::NodeId>>>,
}
...
impl<C: RaftTypeConfig, N: RaftNetworkFactory<C>, S: RaftStorage<C>> Raft<C, N, S> {
    ...
    pub fn new(id: C::NodeId, config: Arc<Config>, network: N, storage: S) -> Self {
        ...
        let inner = RaftInner {
            ...
            core_result: async_lock::Mutex::new(Ok(())),
        };
    ...
    async fn get_core_error(&self) -> Fatal<C::NodeId> {
        ...
        {
            let guard = self.inner.core_result.lock().await;
            match &*guard {
                Ok(()) => {}
                Err(err) => {
                    return err.clone();
        ...
        if let Some(h) = self.inner.raft_handle.lock().await.take() {
            ...
            if let Err(err) = res {
                {
                    let mut guard = self.inner.core_result.lock().await;
                    match &mut *guard {
                        Ok(()) => {}
                        Err(e) => {
                            if err.is_panic() {
