...
impl From<&ExampleStateMachine> for SerializableExampleStateMachine {
    fn from(state: &ExampleStateMachine) -> Self {
        ...
        for (key, value) in state.data.iterator_cf(
            state.data.cf_handle("data").expect("cf_handle"),
        ...
        Self {
            last_applied_log: state.last_applied_log.clone(),
...
pub struct ExampleStateMachine {
    ...
    /// Application data.
    pub data: Arc<rocksdb::DB>,
}

impl ExampleStateMachine {
    fn from_serializable(sm: SerializableExampleStateMachine, db: Arc<rocksdb::DB>) -> Self {
        for (key, value) in sm.data {
            db.put_cf(db.cf_handle("data").unwrap(), key.as_bytes(), value.as_bytes()).unwrap();
        }
        Self {
    ...
    fn insert(&self, key: String, value: String) -> StorageResult<()> {
        self.data
            .put_cf(self.data.cf_handle("data").unwrap(), key.as_bytes(), value.as_bytes())
    ...
    pub fn get(&self, key: &str) -> StorageResult<Option<String>> {
        ...
        self.data
            .get_cf(self.data.cf_handle("data").unwrap(), key)
            .map(|value| {
...
impl ExampleStore {
    pub(crate) async fn new<P: AsRef<Path>>(db_path: P) -> Arc<ExampleStore> {
        ...
        let mut store = Arc::new(ExampleStore { db, state_machine });
        ...
        if let Some(snapshot) = store.get_current_snapshot_().unwrap() {
            ...
            store.state_machine.write().await.last_applied_log = updated_state_machine.last_applied_log;
            store.state_machine.write().await.last_membership = updated_state_machine.last_membership;
