...
impl RaftLogReader<ExampleTypeConfig> for Arc<ExampleStore> {
    async fn get_log_state(&mut self) -> Result<LogState<ExampleTypeConfig>, StorageError<ExampleNodeType>> {
    ...
    async fn try_get_log_entries<RB: RangeBounds<u64> + Clone + Debug + Send + Sync>(
        &mut self,
        range: RB,
    ) -> Result<Vec<Entry<ExampleTypeConfig>>, StorageError<ExampleNodeType>> {
        let log = self.log.read().await;
...
impl RaftStorage<ExampleTypeConfig> for Arc<ExampleStore> {
    ...
    ) -> Result<(), StorageError<ExampleNodeType>> {
