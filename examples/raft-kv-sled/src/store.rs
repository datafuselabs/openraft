...
impl ExampleStateMachine {
    ...
    async fn set_last_membership(&self, membership: EffectiveMembership<ExampleNodeId>) -> StorageResult<()> {
        ...
        state_machine.flush_async().await
            .map_err(m_w_err)
            .map(|_| ())
    }
...
impl ExampleStore {
    ...
    fn get_last_purged_(&self) -> StorageResult<Option<LogId<u64>>> {
        ...
        let val = store_tree.get(b"last_purged_log_id")
    ...
    async fn set_last_purged_(&self, log_id: LogId<u64>) -> StorageResult<()> {
        ...
        store_tree.insert(b"last_purged_log_id", val.as_slice())
            .map_err(s_w_err)?;
    ...
    fn get_snapshot_index_(&self) -> StorageResult<u64> {
        let store_tree = store(&self.db);
        let val = store_tree.get(b"snapshot_index")
            .map_err(s_r_err)?
            .and_then(|v| serde_json::from_slice(&v).ok())
            .unwrap_or(0);
    ...
    async fn set_snapshot_index_(&self, snapshot_index: u64) -> StorageResult<()> {
        ...
        store_tree.insert(b"snapshot_index", val.as_slice())
            .map_err(s_w_err)?;
    ...
    fn get_current_snapshot_(&self) -> StorageResult<Option<ExampleSnapshot>> {
        let store_tree = store(&self.db);
        let val = store_tree.get(b"snapshot")
            .map_err(s_r_err)?
            .and_then(|v| serde_json::from_slice(&v).ok());
...
impl RaftLogReader<ExampleTypeConfig> for Arc<ExampleStore> {
    ...
    async fn try_get_log_entries<RB: RangeBounds<u64> + Clone + Debug + Send + Sync>(
        ...
        let logs_tree = logs(&self.db);
        let logs = logs_tree.range::<&[u8], _>(start.as_slice()..)
            .map(|el_res| {
...
fn logs(db: &sled::Db) -> sled::Tree {
    db.open_tree("logs")
        .expect("logs open failed")
}
fn data(db: &sled::Db) -> sled::Tree {
    db.open_tree("data")
        .expect("data open failed")
}
