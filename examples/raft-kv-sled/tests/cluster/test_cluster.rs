...
use maplit::btreeset;
use openraft::Node;
use raft_key_value_sled::client::ExampleClient;
use raft_key_value_sled::start_example_raft_node;
use raft_key_value_sled::store::ExampleRequest;
...
async fn test_cluster() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _h1 = thread::spawn(move || {
        let x = block_on(async move { start_example_raft_node(1, d1.path().to_str().unwrap(), get_addr(1), get_rpc_addr(1)).await });
        println!("x: {:?}", x);
    });

    let _h2 = thread::spawn(move || {
        let x = block_on(async move { start_example_raft_node(2, d2.path().to_str().unwrap(), get_addr(2), get_rpc_addr(2)).await });
        println!("x: {:?}", x);
    });

    let _h3 = thread::spawn(move || {
        let x = block_on(async move { start_example_raft_node(3, d3.path().to_str().unwrap(), get_addr(3), get_rpc_addr(3)).await });
        println!("x: {:?}", x);
    });
